...
The situation described above also leads to multiple **_VSRs_** in different locations working on a claim before it is concluded. To capture this, we introduced another two measures, the total number of unique **_VSRs_** or adjudicators that work on a claim  `$n_{adjudicators}$` and the total number of unique locations where a claim was processed - `$n_{locations}$`. Finally, we introduce the `$n_{contentions}$` which measures the number of unique claimant contentions at submission.  Submissions with a lot of contentions would likely be more complicated than the ones with fewer contentions. Utilizing the huge dataset available to us, we proceeded to determine these metrics for claims established using manual methods and the **_AI_** based **_CAAPIv1.0/BCAPI_**. 
